...
- 2025-11-19: Critical Timing & Synchronization Fixes + Final Retry from File ✅
  - **Root Cause Fixed**: Eliminated premature data extraction before JavaScript loads metadata
  - **Robust 15-second timeout**: Replaced short 3-5 second waits with content-aware 15-second timeouts
  - **Cookie banner dismissal**: Automatically detects and dismisses cookie consent banners to prevent JavaScript blocking
  - **Enhanced Shadow DOM support**: Added waitForMetadataFields() method that specifically waits for Shadow DOM content to be populated
  - **Reduced concurrency**: Lowered from 5 to 2 concurrent tabs for improved stability during testing
  - **Content-aware waits**: New waitForContentLoaded() method ensures elements contain non-empty text before extraction
  - **Configurable timeouts**: Added metadata.metadataTimeout setting in scraper.config.json (default: 15000ms)
  - **Proper config fallback**: Fixed concurrency to read from scraper.config.json when job config doesn't specify it
  - **Final retry from file**: Before completing, scraper reads failed-scrapes.txt and attempts one final retry
  - **Auto-cleanup on success**: Successfully retried images are automatically removed from failed-scrapes.txt
  - **Key methods added**: dismissCookieBanner(), waitForContentLoaded(), extractFromShadowDOM(), waitForMetadataFields(), readFailuresFromFile(), removeFromFile()
  - These changes address systematic metadata extraction failures caused by timing race conditions in dynamic JavaScript-heavy SmartFrame pages
